...
const Calendar = (props) => {
    ...
    const renderWeek = (days, id) => {
        let takedays = days.slice(0,5);
        
        const week = [];
        takedays.forEach((day, id2) => {
            week.push(renderDay(day, id2));
        }, this);
        if (props.showWeekNumbers) {
            week.unshift(renderWeekNumber(takedays[takedays.length - 1].getWeek()));
        }
    ...
    const direction = () => {
        ...
        const figureHorizontalDirection = (delta,deltax) => {
        ...
        return Math.abs(dx) > Math.abs(dy) ? figureHorizontalDirection(dx,locatioX) : figureVerticalDirection(dy);
    };
    ...
    onTouchEnd={(event) => {
        ...
        locatioX = event.nativeEvent.locationX;                    
